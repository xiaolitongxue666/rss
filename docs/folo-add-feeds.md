...
**若在 FOLO 中只能看到约 30 分钟前的动态**：多半是 FOLO 对该订阅的**刷新间隔**较长（例如 30 分钟）。可尝试：在 FOLO 中把该订阅的刷新间隔调短（如 5–10 分钟），或进入该订阅后**手动下拉刷新**以立即拉取最新内容。若自建 RSSHub 且启用了缓存，可在服务器 `.env` 中适当调低 `CACHE_EXPIRE`（如 60）后重启 rsshub，以缩短服务端缓存时间。
