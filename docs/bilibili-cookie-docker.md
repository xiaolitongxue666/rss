...
- **Cookie 与 uid**：`BILIBILI_COOKIE_<uid>` 的 uid 为**你本人** B 站账号 UID；`/followings/video/:uid` 的 uid 必须与某条已配置的 Cookie 的 uid 一致。配置后执行 `./scripts/apply-bilibili-cookie.sh --uid <你的uid> --remote` 推送到服务器。
...
- **一键部署到远程**：执行 `./scripts/apply-bilibili-cookie.sh --uid <uid> --remote`（见上文 [3.5 从 cookie 目录生成并应用到 .env](#35-从-cookie-目录生成并应用到-env推荐) 远程用法）。
